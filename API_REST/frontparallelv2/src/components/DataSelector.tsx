...
const DataSelector: React.FC = () => {
  ...
  useEffect(() => {
    setLoading(true);
    fetch('/api//github/organizations')  // Endpoint que devuelve la lista de organizaciones
  ...
  const handleAddOrganization = () => {
    if (newOrg) {
      ...
      fetch(`/api//github/org-users2/${newOrg}`, {
        method: 'GET',  // O el método que utilices para crear los usuarios
      })
        .then(response => {
          if (response.ok) {
            // Refrescar la lista de organizaciones
            return fetch('/api//github/organizations');  // Vuelve a obtener la lista actualizada
          } else {
