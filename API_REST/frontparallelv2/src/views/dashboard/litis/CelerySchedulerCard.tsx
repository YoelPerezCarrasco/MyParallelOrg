...
const CelerySchedulerCard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch('http://localhost:8000/adminml/get_celery_schedule', {
          headers,
        });
        if (response.ok) {
        ...
        const historyResponse = await fetch('http://localhost:8000/adminml/update_history', {
          headers,
        });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/adminml/update_celery_schedule', {
        method: 'POST',
        headers,
